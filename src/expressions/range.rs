...
pub fn compute_max_keyword_size(max_size: usize) -> usize {
    2 * max_size
}
...
pub fn gen_ciphertext_for_range_search<E: Engine, F: BaseROFr<E>, R: RngCore>(
    ...
    let max_keyword_size = compute_max_keyword_size(max_size);
    let bits = uint2bits(val, max_size);
    let sliced_uints = (0..max_size)
        .map(|i| {
...
pub fn gen_trapdoor_for_range_search<E: Engine, F: BaseROFr<E>, R: RngCore>(
    ...
    let max_keyword_size = compute_max_keyword_size(max_size);
    let nodes = get_canonical_cover_nodes(min, max, max_size);
...
fn get_canonical_cover_nodes(min: u64, max: u64, max_size: usize) -> HashSet<Vec<bool>> {
    let n1 = max - min + 1;
    ...
    for i in 0..l {
        allowed_lens.insert(max_size - i);
    }
    let n2_bits = uint2bits(n2, max_size);
    ...
    while !allowed_lens.is_empty() {
        let next_node = get_canonical_next_max(min, max, max_size, &allowed_lens);
        min = compute_max_uint(&next_node, max_size) + 1;
...
fn get_canonical_next_max(
    ...
    let mut bits_pattern = uint2bits(min, max_size);
    let min_size_in_set = *allowed_lens.iter().min().unwrap();
    while bits_pattern.len() >= 2
        && (compute_min_uint(&bits_pattern[0..bits_pattern.len() - 1], max_size) >= min)
        && (compute_max_uint(&bits_pattern[0..bits_pattern.len() - 1], max_size) <= max)
...
fn compute_min_uint(bits: &[bool], max_size: usize) -> u64 {
    let mut min_bits = bits.to_vec();
    min_bits.append(&mut vec![false; max_size - bits.len()]);
    bits2uint(&min_bits)
}

fn compute_max_uint(bits: &[bool], max_size: usize) -> u64 {
    let mut max_bits = bits.to_vec();
    max_bits.append(&mut vec![true; max_size - bits.len()]);
    bits2uint(&max_bits)
}
...
mod test {
    ...
    fn test_valid_range_case_simple() {
        ...
        let max_size = 5;
        let n = compute_max_keyword_size(max_size);
